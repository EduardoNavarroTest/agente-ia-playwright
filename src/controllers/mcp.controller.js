...
import { findBestMatchingTest, detectarIntencion, responseChat, procesarConsulta, formatearResultadoTest } from '../services/gemini.service.js';
...
async function handlePrompt(req, res) {
    ...
    try {
        ...
        for(const test of lista) {
            console.log(`[TEST] Título: ${test.title}, Archivo: ${test.file}`);
        }
        ...
        if (found) {
            ...
            console.log('[NO MATCH] No se encontró un test coincidente para el prompt:', prompt);
        ...
        console.error('[HANDLE PROMPT ERROR]', error.message);
        return res.status(500).json({
            jsonrpc: '2.0',
            id,
